...
We define ranges for the two glmnet tuning parameters, alpha (mixture) and lambda (penalty). 

We also set a vector of penalty weights, which will apply equal penalty weighting to all variables except `bdi_baseline`, which will have a penalty weight of 0 and consequently will be included in all models.
```{r}
algorithm <- "glmnet"
...
NOW DONE WITHIN FUN_MOODIVATE.R - GEF update documentation
