...
```{r}
d <- read_csv("file_name.csv", show_col_types = FALSE)
```
...
d <- d |> 
  ...
  mutate(y = factor(y, levels = c(!!y_level_pos,
                                  !!y_level_neg))) |> 
  # character -> factor & set factor levels
  mutate(across(c(),
                ~ factor(., levels = c(),
                         ordered = TRUE))) |> 
  mutate(across(c(),
                ~ factor(., levels = c(),
                         ordered = FALSE))) |> 
  # make any variables numeric that need it
  mutate(across(c(),
                ~ as.numeric(.)))
...
```{r}
rec <- recipe(y ~ ., data = d) |> 
  step_rm(subid) |> # or equivalent variable
  ...
  # remove any features where there is still missingness (backup)
