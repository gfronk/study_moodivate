...
metrics_avg <- read_csv(
  "~/Desktop/dahne_lab/moodivate/metrics_inner_avg.csv",
...
In the case of ties (i.e., equal AUC across multiple configurations in an outer split and feature set), we:
...
metrics_out <- all |> 
  ...
  list_rbind() |> 
  write_rds("~/Desktop/dahne_lab/moodivate/outer_metrics.rds")

preds_out <- all |> 
  map(\(l) pluck(l, "probs_out")) |> 
  list_rbind() |> 
  write_rds("~/Desktop/dahne_lab/moodivate/outer_preds.rds")
...
```{r default_cm}
(cm <- preds_out |> 
   mutate(estimate = if_else(prob_raw > .5, "Non-Responder", "Responder"),
...
(cm <- preds_out |> 
   mutate(estimate = if_else(prob_raw > j_thres_roc, 
