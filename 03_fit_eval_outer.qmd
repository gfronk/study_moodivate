...
We select the best model configuration for each outer fold for each feature set. Specifically, this is the model configuration that produces the highest average performance across *validation sets* (held-out inner folds). 
```{r best_model_2}
