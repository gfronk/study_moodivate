...
d <- read_csv("~/Desktop/musc/dahne_lab/moodivate/data_clean.csv", 
  ...
  # class BDI outcome variable into clearly labeled levels
  ...
  # class PHQ8 outcome variable into clearly labeled levels
  mutate(phq_outcome_cat = if_else(phq8_change_bl_w12 > -5, 
                                   "NonResponder", 
                                   "Responder", 
                                   NA_character_)) |> 
  mutate(phq_outcome_cat = factor(phq_outcome_cat)) |> 
  # mutate(phq_outcome_cat_loose = if_else(phq8_change_bl_w12 <= -4,
  #                                        "Responder",
  #                                        "NonResponder",
  #                                        NA_character_)) |> 
  rename(phq_outcome_cont = phq8_change_bl_w12, phq_baseline = phq8_score_bl) |> 
  # remove PHQ8 scores from other timepoints
  select(-starts_with("phq8_score"), -starts_with("phq8_change")) |> 
  ...
  relocate(record_id, 
           bdi_baseline, bdi_outcome_cat, bdi_outcome_cont,
           phq_baseline, phq_outcome_cat, phq_outcome_cont,
           starts_with("session"), starts_with("total"),
           starts_with("average"), starts_with("add_act"),
           starts_with("schedule_"), starts_with("scheduled_"),
           starts_with("completed_"), starts_with("add_g"),
           starts_with("badges"), starts_with("phq8_comp"),
           starts_with("mood")) |> 
  filter(!is.na(bdi_outcome_cat)) |> # NAs overlap fully on both outcomes
...
d <- d |> 
  mutate(bdi_outcome_quartile = case_when(
    bdi_outcome_cont <= iqr1[2] ~ "0-25%",
    bdi_outcome_cont <= iqr1[3] ~ "26-50%",
    bdi_outcome_cont <= iqr1[4] ~ "51-75%",
    bdi_outcome_cont <= iqr1[5] ~ "76-100%",
    TRUE ~ NA_character_
  )) 
```

Create BDI baseline quartiles categorical variable
```{r}
iqr2 <- quantile(d$bdi_baseline)

d <- d |> 
  mutate(bdi_baseline_quartile = case_when(
    bdi_baseline <= iqr2[2] ~ "0-25%",
    bdi_baseline <= iqr2[3] ~ "26-50%",
    bdi_baseline <= iqr2[4] ~ "51-75%",
    bdi_baseline <= iqr2[5] ~ "76-100%",
    TRUE ~ NA_character_
  )) 
```

Create PHQ outcome quartiles categorical variable
```{r}
iqr3 <- quantile(d$phq_outcome_cont)

d <- d |> 
  mutate(phq_outcome_quartile = case_when(
    phq_outcome_cont <= iqr1[2] ~ "0-25%",
    phq_outcome_cont <= iqr1[3] ~ "26-50%",
    phq_outcome_cont <= iqr1[4] ~ "51-75%",
    phq_outcome_cont <= iqr1[5] ~ "76-100%",
    TRUE ~ NA_character_
  )) 
```

Create PHQ baseline quartiles categorical variable
```{r}
iqr4 <- quantile(d$phq_baseline)

d <- d |> 
  mutate(phq_baseline_quartile = case_when(
    phq_baseline <= iqr2[2] ~ "0-25%",
    phq_baseline <= iqr2[3] ~ "26-50%",
    phq_baseline <= iqr2[4] ~ "51-75%",
    phq_baseline <= iqr2[5] ~ "76-100%",
    TRUE ~ NA_character_
  )) |> 
  relocate(starts_with("bdi_baseline"), starts_with("bdi_outcome"), 
           starts_with("phq_baseline"), starts_with("phq_outcome"),
           everything())
```
...
d_long <- d |> 
  ...
  relocate(record_id, week, everything()) |> 
  glimpse()
```
...
d_long <- d_long |> 
  set_variable_labels(
    ...
    phq_baseline = "PHQ Baseline",
    phq_baseline_quartile = "PHQ Baseline Quartile",
    phq_outcome_cat = "Response Category (PHQ)",
    phq_outcome_cont = "Raw Change from Baseline (PHQ)",
    phq_outcome_quartile = "PHQ Outcome Quartile",
    week = "Week",
...
## BDI Categorical Outcome (Responder vs. Non-Responder)
...
#   add_p(test.args = c("mood_days_wk11", "mood_days_wk2",
#                       "mood_days_wk7", "mood_days_wk8") ~ list(
...
#   summarize(across(c(bdibaseline, sessioncount:phq8comp),
#                    c(mean, sd))) |> 
...
#   mutate(Predictor = c("BDI Baseline", rep("", 133))) |> 
...
## PHQ8 Categorical Outcome (Responder vs. Non-Responder)
...
```{r}
tblsum_cat_object <- d |>
  tbl_summary(by = phq_outcome_cat,
              include = !c(record_id, bdi_baseline, contains("outcome"), contains("quartile")),
              digits = all_continuous() ~ 2
  ) |>
  add_overall() |>
  modify_header(label ~ "Predictor") |>
  modify_spanning_header(c("stat_1", "stat_2") ~ "Response Category (PHQ8)") |>
  add_p(test.args = c("mood_days_wk12", "mood_days_wk2",
                       "mood_days_wk4", "mood_days_wk6",
                      "mood_days_wk7", "mood_days_wk8",
                      "mood_days_wk9") ~ list(
                         simulate.p.value = TRUE
                       ),
        pvalue_fun = label_style_pvalue(digits = 3))
...
d_sum_group <- d_long |> 
  rename_with(~ str_replace_all(.x, "_", "")) |> 
  group_by(bdioutcomecat, week) |> 
  summarize(across(c(phqbaseline, sessioncount:phq8comp),
                   c(mean, sd))) |> 
  rename_with(~ str_replace(.x, "_1", "_mean")) |> 
  rename_with(~ str_replace(.x, "_2", "_sd")) |> 
  mutate(across(c(ends_with("mean"), ends_with("sd")),
...
d_table <- d_table |> 
  mutate(Predictor = 
           case_match(Predictor,
                      ...
                      "mooddays" ~ "Number of Days Mood was Recorded",
                      "phq8comp" ~ "Completed PHQ8")) |> 
  filter(!(Predictor == "PHQ8 Baseline" & week %in% c("Week 2", "Week 3", "Week 4",
                                                     "Week 5", "Week 6", "Week 7",
                                                     "Week 8", "Week 9", "Week 10",
                                                     "Week 11", "Week 12"))) |> 
  mutate(week = case_when(
    Predictor == "PHQ8 Baseline" ~ NA_character_,
    TRUE ~ week
  ))
...
tbl_final <- d_table_with_stat |> 
  ...
  ungroup() |> 
  select(Predictor, Week, contains("M (SD)"), `Group Difference`) |> 
  mutate(Predictor = c("PHQ8 Baseline", rep("", 132))) |> 
  add_row(Predictor = "Completed PHQ8",
          .before = 122) |>
  add_row(Predictor = "Number of Days Mood was Recorded",
          .before = 110) |>
