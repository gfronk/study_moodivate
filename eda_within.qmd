...
Write out data including within subs variables

Add phq8 interim data back in
```{r}
dphq <- read_csv("~/Desktop/musc/dahne_lab/moodivate/data_clean.csv", 
              show_col_types = FALSE) |> 
  select(record_id, starts_with("phq8_change")) |> 
  filter(!is.na(phq8_change_bl_w12)) |> 
  select(-phq8_change_bl_w12)
  
```

```{r}
d |> 
  left_join(dphq, by = "record_id") |> 
  write_csv("~/Desktop/musc/dahne_lab/moodivate/data_within_clean.csv")
...
## Descriptives tables: BDI

### Make quartile variables (divide continuous into quartiles)
...
<!-- ## Visualize continuous relationships: BDI -->
...
tbl_session_count <- d |>
  ...
  modify_header(label ~ "Predictor") |>
  modify_spanning_header(c("stat_1", "stat_2") ~ "PHQ8 Category") |>
  add_p(pvalue_fun = label_style_pvalue(digits = 3))
...
tbl_phq8_comp <- d |>
  tbl_summary(by = phq_outcome_cat,
              include = c(contains(str_c(stem_vars[10], "_chg"))),
              digits = all_continuous() ~ 2
  ) |>
  modify_header(label ~ "Predictor") |>
  modify_spanning_header(c("stat_1", "stat_2") ~ "PHQ8 Category") |>
...
tbl_mood_days <- d |>
  tbl_summary(by = phq_outcome_cat,
              include = c(contains(str_c(stem_vars[11], "_chg"))),
              digits = all_continuous() ~ 2
  ) |>
  modify_header(label ~ "Predictor") |>
  modify_spanning_header(c("stat_1", "stat_2") ~ "PHQ8 Category") |>
  add_p(test.args = c("mood_days_chg_from_prev_wk3", "mood_days_chg_from_prev_wk7",
                      "mood_days_chg_from_prev_wk8") ~ list(
